...
void inserir(Fila *f, Livro l) {
  if (f->nItens < f->capacidade) {
      ...
      printf("Fila esta cheia. A capacidade e de apenas: 50 livros.\n");
  }
}
...
Livro removerPorTitulo(Fila *f, const char *titulo) {
    ...
    if (encontrado) {
        ...
        printf("Livro com o titulo \"%s\" nao foi encontrar na fila.\n", titulo);
    }
...
void empilhar(struct Pilha *p, Livro l, const char* arquivo) {
    if (p->topo < p->capacidade - 1) {
        if (strlen(l.autor) > 0 && strlen(l.titulo) > 0) {
            ...
            if (fPilha == NULL) {
                printf("Erro para abrir o arquivo da pilha.\n");
                exit(1);
            }
            ...
            printf("O livro esta vazio. Nao e possivel empilhar.\n");
        }
    } else {
        printf("Fila esta cheia. A capacidade e de apenas: 50 livros.\n");
    }
...
void removerLivroPorTituloDaPilha(Pilha *p, const char* arquivo) {
    if (pilhaVazia(p)) {
        printf("A pilha esta vazia. Nao foi possivel remover.\n");
        return;
    }
    ...
    if (indice == -1) {
        printf("Livro com o titulo \"%s\" nao foi encontrado na pilha.\n", titulo);
    } else {
        ...
        printf("O livro foi removido da pilha: Autor: %s, Titulo: %s\n", livroRemovido.autor, livroRemovido.titulo);
    }
}
...
int main() {
  ...
  do{
    printf("\n===Menu===\n");
    printf("1. Mostrar a fila de livros\n");
    printf("2. Adicionar um livro a fila\n");
    printf("3. Adicionar um livro a pilha\n");
    ...
    switch(op){
      case 1:
        ...
        if(estaVazia(&f)){
          ...
          printf("A fila esta cheia. A capacidade e de apenas: 50 livros.\n");
        }else{
      ...
      case 3:
        ...
        if (strlen(autor) > 0 && strlen(titulo) > 0) {
            ...
            if (!pilhaCheia(&p)) {
                ...
                printf("O livro foi empilhado com sucesso!\n");
            } else {
                printf("A pilha esta cheia. A capacidade e de apenas: 50 livros.\n");
            }
        } else {
      ...
      case 4:
        limparTela();
        if (!estaVazia(&f)) {
            printf("Informe o título do livro a ser removido: ");
            getchar();
            fgets(titulo, sizeof(titulo), stdin);
            titulo[strcspn(titulo, "\n")] = '\0';
            ...
            if (strcmp(livroRemovido.titulo, "") != 0) {
                printf("Livro removido: Autor: %s, Titulo: %s\n", livroRemovido.autor, livroRemovido.titulo);
            }
        } else {
            printf("A fila esta vazia. Nao e possível remover.\n");
        }
        break;
      case 5:
        limparTela();
